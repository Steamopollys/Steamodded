...
function SMODS.injectSprites()
	...
	for i, sprite in ipairs(SMODS.Sprites) do
		...
		sendInfoMessage("The Sprite named " .. sprite.name .. " with path " .. sprite.path .. " have been registered.", "SteamoddedSprite")
	end
    ...
    sendInfoMessage("All the sprites have been loaded!", "SteamoddedSprite")
end
        ...
        if _center.set then
            if (_center.set == 'Joker' or _center.consumeable or _center.set == 'Voucher') and _center.atlas then
                if self.params.bypass_discovery_center or (_center.unlocked and _center.discovered) then
                    ...
                    sendDebugMessage(inspect(self.children.center), "SteamoddedSprite")
