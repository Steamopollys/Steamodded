	...
	if mod_has_achievement then table.insert(mod_tabs, 
		{
			...
			tab_definition_function = function()
				...
				G.SMODS_ACHIEVEMENTS_MATRIX = nil
...
function buildAchievementsTab(mod, args)
	args = args or {}
	fetch_achievements()
	...
	if not G.SMODS_ACHIEVEMENTS_MATRIX then
		G.SMODS_ACHIEVEMENTS_MATRIX = {{},{}}
		...
		for i = 1, achievements_per_row*2 do
			...
			table.insert(G.SMODS_ACHIEVEMENTS_MATRIX[row], {
				n = G.UIT.C,
			...
			if #G.SMODS_ACHIEVEMENTS_MATRIX[row] == achievements_per_row then 
				row = row + 1
				G.SMODS_ACHIEVEMENTS_MATRIX[row] = {}
			end
		end
	end
	...
	local t = {
		...
		{n=G.UIT.R, config={align = "cm"}, nodes={
			{n=G.UIT.R, config={align = "cm"}, nodes=G.SMODS_ACHIEVEMENTS_MATRIX[1]},
			{n=G.UIT.R, config={align = "cm"}, nodes=G.SMODS_ACHIEVEMENTS_MATRIX[2]},
			create_option_cycle({options = achievements_options, w = 4.5, cycle_shoulders = true, opt_callback = 'achievments_tab_page', focus_args = {snap_to = true, nav = 'wide'},current_option = args.selected_tab or 1, colour = G.C.RED, no_pips = true})
		}}
		}}
	}}}
	return {
...
G.FUNCS.achievments_tab_page = function(args)
	...
	local achievements_pool = {}
	for k, v in pairs(G.ACHIEVEMENTS) do
		if v.mod and v.mod.id == G.ACTIVE_MOD_UI.id then achievements_pool[#achievements_pool+1] = v end
	end
	local achievement_tab = {}
	for k, v in pairs(achievements_pool) do
		achievement_tab[#achievement_tab+1] = v
	...
	tab_contents.config.object = UIBox{
		definition = buildAchievementsTab(G.ACTIVE_MOD_UI, {selected_tab = args.cycle_config.current_option}),
