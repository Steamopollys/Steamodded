--- STEAMODDED HEADER
--- MOD_NAME: Ortalab Editions
--- MOD_ID: OrtalabEditions
--- MOD_AUTHOR: [Eremel_]
--- MOD_DESCRIPTION: Adds the editions for the Ortalab mod.
--- BADGE_COLOUR: 3FC7EB
----------------------------------------------
------------MOD CODE -------------------------
...
local test_deck_def = {
    ["name"] = "Test Deck",
    ["text"] = {
        "Start with a {C:spectral}High Exposure{}",
        "and a {C:spectral}Neon{}."
    }
}

function SMODS.INIT.OrtalabEditions()
    local mod = SMODS.findModByID("OrtalabEditions")
    ...
    local greyDef = {
        ...
        shader_path = mod.path .. "/assets/shaders/greyscale.fs",
    ...
    function SMODS.Spectrals.c_grey.can_use(card)
        if G.STATE ~= G.STATES.HAND_PLAYED and G.STATE ~= G.STATES.DRAW_TO_HAND and G.STATE ~= G.STATES.PLAY_TAROT or
            any_state then
            if next(SMODS.Edition:get_editionless_jokers({})) then
                return true
    ...
    function SMODS.Spectrals.c_grey.use(card, area, copier)
        local used_tarot = (copier or card)
        local eligible_jokers = SMODS.Edition:get_editionless_jokers({})
        G.E_MANAGER:add_event(Event({
            trigger = 'after',
            ...
            func = function()
                local over = false
                local selected_joker = pseudorandom_element(eligible_jokers, pseudoseed('seed'))
                selected_joker.set_edition(selected_joker, { sepia = true })
                used_tarot:juice_up(0.3, 0.5)
                return true
            end
        }))
    ...
    function SMODS.Spectrals.c_grey.loc_def(self, info_queue)
        info_queue[#info_queue+1] = G.P_CENTERS.e_greyscale
        return {}
     end

    SMODS.Deck:new("Test Deck", "test_deck", {
        consumables = {'c_high_exposure', 'c_neon'}
    }, {
        x = 0,
        y = 0
    }, test_deck_def):register()
